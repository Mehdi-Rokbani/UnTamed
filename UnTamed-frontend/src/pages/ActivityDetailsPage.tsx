    ...
    >
      ...
      >
        ...
        <form onSubmit={handleSubmit}>
          ...
          {error && (
            <p style={{ marginTop: 10, color: "#b42318", fontWeight: 600, fontSize: 13 }}>
              {error}
            </p>
          )}
...
function ParticipantsModal({
  ...
  preview: ParticipantsPreviewResponse;
  onClose: () => void;
}) {
  return (
...
export default function ActivityDetailsPage() {
  ...
  const spotsLeft = useMemo(() => {
    ...
    return Math.max(0, selectedSession.capacity - selectedSession.bookedCount);
  }, [selectedSession]);
  ...
  const allImages = useMemo(() => {
    ...
    if (coverImageUrl) {
      return [{ url: coverImageUrl, cover: true, order: 0, alt: templateTitle }];
    }
    return [];
  ...
  const guideOwnerId = useMemo(
    () =>
      ((selectedSession as any)?.guideId ??
        tplAny?.guideId ??
        guide?.id ??
        null) as string | null,
                  ...
                  {upcomingSessionsCount > 0 && (
                    <>
                      ...
                      <span className={styles.metaText}>
                        {upcomingSessionsCount} upcoming dates
                      </span>
                    </>
                  )}
                </div>
                  ...
                  <div className={styles.guideAvatarWrap}>
                    {guide.profileImageUrl ? (
                      ...
                      <div className={styles.guidePlaceholder}>
                        {String(guide.username ?? "?")[0].toUpperCase()}
                      </div>
                    )}
                  ...
                  <div className={styles.quickItem}>
                    ...
                    <div>
                      ...
                      <div className={styles.quickVal}>
                        {spotsLeft}/{selectedSession.capacity}
                      </div>
                    </div>
                  </div>
                )}
                ...
                {bookingStep === "success" && bookingSuccessId ? (
                  <div className={styles.successState}>
                    ...
                    <p className={styles.successSub}>
                      ...
                      <strong>
                        {selectedSession ? formatDateShort(selectedSession.startAt) : "your adventure"}
                      </strong>
                    </p>
                    ...
                    <div className={styles.successId}>
                      Booking #{bookingSuccessId.slice(-8).toUpperCase()}
                    </div>
                  ...
                  <>
                      ...
                      <div className={styles.sidebarField}>
                        ...
                        <div className={styles.peopleStepper}>
                          ...
                          <button
                            ...
                            type="button"
                            onClick={() =>
                              setPeople((p) => Math.min(Math.min(10, spotsLeft ?? 10), p + 1))
                            }
                            disabled={
                              people >= Math.min(10, spotsLeft ?? 10) ||
                              !selectedSession ||
                              (spotsLeft ?? 0) <= 0
                            }
                          >
                    ...
                    <button
                      className={`${styles.bookBtn} ${
                        bookingStep === "confirming" ? styles.bookBtnLoading : ""
                      }`}
                      type="button"
                    ...
                    {user && isGuideOwner && (
                      <p className={styles.noCharge}>You can't book your own activity.</p>
                    )}
                    {user && isWithinCutoff && (
                          ...
                          <div className={styles.whoJoiningText}>
                            ...
                            {participantsPreview.seatsLeft != null && (
                              <div className={styles.whoJoiningSeats}>
                                {participantsPreview.seatsLeft} seat
                                {participantsPreview.seatsLeft !== 1 ? "s" : ""} left
                              </div>
                            )}
                          </div>
